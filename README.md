...
### Menu bar hiding on macOS 27

ShimKit uses bounded spacers on macOS 27 to send hidden icons into the native **«** overflow menu. Earlier macOS versions retain the original wide-spacer behavior. The first launch after updating stays in arrangement mode: Command-drag icons to the left of ShimKit’s divider, keep its arrow on the right, then click **Hide Icons**. New stable control identities allow macOS to remember this layout on later launches. Option-click the arrow or choose **Show All to Arrange** to reveal both sections.

The optional always-hidden section is hidden from the main bar during normal reveal; macOS can still expose it through its native overflow menu. Hidden icons are not private or disabled. Spacer sizing accounts for the notch and different display widths, with up to seven bounded items per section. Extremely different display widths can exceed that bounded span; use Show All to Arrange when adjusting a layout.

The compatibility implementation was informed by the documented limits in [MenubarHide](https://github.com/junior-rj/menubar-hide#how-it-works). The implementation is independent. Validation on macOS 27 included repeated collapse/reveal, native overflow access, always-hidden and Show All transitions, disable, and cleanup through an isolated AppKit test app using ShimKit’s controller. Automated tests cover notch and mixed-width sizing and retain earlier-system geometry checks. Multiple physical displays and earlier macOS versions were not available for live UI testing.
